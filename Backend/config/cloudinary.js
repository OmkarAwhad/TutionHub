...
const extractPublicIdFromUrl = (cloudinaryUrl) => {
   ...
   try {
      ...
      const uploadMatch = pathname.match(/\/(image|video|raw)\/upload\/(?:v\d+\/)?(.+)$/);
      if (uploadMatch) {
...
const deleteFileFromCloudinary = async (fileUrl) => {
   ...
   try {
      ...
      console.log(`Attempting to delete: ${publicId} with resource_type: ${resourceType}`);
      ...
      try {
         ...
         if (result && result.result === "ok") {
            ...
            console.log("File was not found in Cloudinary (may have been already deleted)");
            return true; // Consider this a success since the file is gone
         }
      } catch (deleteError) {
         console.log(`Failed with resource_type ${resourceType}:`, deleteError.message);
      }
      ...
      for (const type of resourceTypes) {
         ...
         try {
            ...
            if (result && (result.result === "ok" || result.result === "not found")) {
               console.log(`Successfully deleted with resource_type: ${type}`);
               return true;
            }
         } catch (error) {
            console.log(`Failed with resource_type ${type}:`, error.message);
         }
      }
      ...
      try {
         ...
         if (adminResult && adminResult.deleted && adminResult.deleted[publicId] === "deleted") {
   ...
   try {
      ...
      for (const variation of variations) {
         for (const resourceType of resourceTypes) {
            try {
               console.log(`Trying variation: ${variation} with type: ${resourceType}`);
               ...
               if (result && (result.result === "ok" || result.result === "not found")) {
                  console.log(`Success with variation: ${variation}, type: ${resourceType}`);
                  return true;
               }
            } catch (error) {
