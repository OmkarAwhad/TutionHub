...
module.exports.getMarksForEdit = async (req, res) => {
   try {
      ...
      if (lecture.description !== "Test") {
         return res.json(
            new ApiError(400, "Marks can only be edited for Test lectures")
         );
      }

      // Get all students who were present for this lecture
      const attendanceRecords = await require("../models/attendance.model").find({
         lecture: lectureId,
         status: "Present"
      ...
      const studentsWithMarks = attendanceRecords.map(attendance => ({
         ...
         totalMarks: marksMap[attendance.student._id.toString()]?.totalMarks || 0,
         description: marksMap[attendance.student._id.toString()]?.description || ""
      }));

      // Get total marks from first record or default
      const defaultTotalMarks = marksRecords.length > 0 ? marksRecords[0].totalMarks : 0;
      ...
      return res.json(
         new ApiError(500, "Error fetching marks for edit: " + error.message)
      );
   }
};
...
module.exports.updateMarksInBulk = async (req, res) => {
   try {
      ...
      if (!marksData || !Array.isArray(marksData)) {
         return res.json(new ApiError(400, "Valid marks data is required"));
      }

      if (!totalMarks || totalMarks <= 0) {
         return res.json(new ApiError(400, "Valid total marks is required"));
      }
      ...
      if (lecture.description !== "Test") {
         return res.json(
            new ApiError(400, "Marks can only be updated for Test lectures")
         );
      }
      ...
      const marksPromises = marksData
         .filter(data => data.marks !== null && data.marks !== undefined) // Only create records for marked students
         .map(async (data) => {
            if (data.marks > totalMarks) {
               throw new Error(`Marks ${data.marks} cannot be greater than total marks ${totalMarks}`);
            }
   ...
   try {
      ...
      marksDetails.forEach((mark) => {
         ...
         subjectWiseData[subjectId].tests.push({
            ...
            percentage: ((mark.marks / mark.totalMarks) * 100).toFixed(1),
            date: mark.createdAt,
      ...
      Object.keys(subjectWiseData).forEach(subjectId => {
         ...
         subject.average = ((subject.totalObtained / subject.totalPossible) * 100).toFixed(1);
         ...
         if (subject.tests.length >= 2) {
            ...
            const firstPercentage = (firstTest.marks / firstTest.totalMarks) * 100;
            const lastPercentage = (lastTest.marks / lastTest.totalMarks) * 100;
            subject.improvement = (lastPercentage - firstPercentage).toFixed(1);
         }
      });

      const overallPercentage = ((totalMarksObtained / totalMarksPossible) * 100).toFixed(1);
      ...
      return res.json(
         new ApiResponse(200, {
            ...
            overallStats: {
               ...
               totalSubjects: Object.keys(subjectWiseData).length
            },
...
module.exports.getPerformanceComparison = async (req, res) => {
   try {
      ...
      const comparisonData = studentMarks.map(studentMark => {
         ...
         const classAverage = classData ? (classData.totalMarks / classData.totalStudents).toFixed(1) : 0;
         const studentPercentage = ((studentMark.marks / studentMark.totalMarks) * 100).toFixed(1);
         const classPercentage = classData ? ((classAverage / studentMark.totalMarks) * 100).toFixed(1) : 0;
         ...
         return {
            ...
            performanceDiff: (studentPercentage - classPercentage).toFixed(1)
         };
      });

      return res.json(
         new ApiResponse(200, comparisonData, "Performance comparison fetched successfully")
      );
   } catch (error) {
