...
function AttendForSub({ subjects }) {
      ...
      const fetchAttendance = async () => {
         ...
         for (const subject of subjects) {
            try {
               ...
               console.log(`Attendance data for ${subject.name}:`, result);
               if (result) {
   ...
   return (
      <div className="p-4">
         {loading ? (
            <p className="text-richblack-500">Loading attendance data...</p>
         ) : subjects && subjects.length > 0 ? (
            <table className="w-full text-left border-collapse">
               <thead>
                  <tr className="bg-gray-200 text-richblack-900">
                     <th className="p-3 border-b">Subject Name</th>
                     <th className="p-3 border-b">Total Lectures</th>
                     <th className="p-3 border-b">Unrecorded</th>
               ...
               <tbody>
                  {subjects.map((subject) => (
                     <tr key={subject._id} className="hover:bg-richblack-50  ">
                        <td className="p-3 border-b">
                           {subject.name}
                        </td>
                        <td className="p-3 border-b">
                           {attendanceData[subject._id]?.statistics?.totalLectures ?? "-"}
                        </td>
                        <td className="p-3 border-b">
                           {attendanceData[subject._id]?.statistics?.unrecorded ?? "-"}
                        </td>
                        <td className="p-3 border-b">
                           {attendanceData[subject._id]?.statistics?.present ?? "-"}
                        </td>
                        <td className="p-3 border-b">
                           {attendanceData[subject._id]?.statistics?.absent ?? "-"}
                        </td>
                        <td className="p-3 border-b">
                           {attendanceData[subject._id]?.statistics?.percentage ?? "-"}
                        </td>
                     </tr>
                  ))}
               </tbody>
