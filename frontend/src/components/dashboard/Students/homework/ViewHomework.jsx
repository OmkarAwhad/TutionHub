...
import { submitHomework } from "../../../../services/operations/homework.service";
...
function ViewHomework() {
   ...
   const handleUpload = async (file, trackProgress) => {
      ...
      try {
         ...
         if (response.success) {
            toast.success(
               response.message || "Homework submitted successfully!"
            );
            setSubmission(response.data);
            return response.data;
         } else {
            toast.error(
               response.message || "Failed to submit homework."
            );
            throw new Error(
               response.message || "Failed to submit homework"
            );
   ...
   return (
      <div className="px-6 mx-auto max-w-4xl">
         ...
         <div className="shadow-lg rounded-xl border border-gray-200 px-6 py-8 space-y-6">
            ...
            {/* File Preview section - only show if there's a file */}
            {/* {homework.fileUrl && (
               <div className="pt-4">
                  <h4 className="text-medium-gray font-medium mb-3">
                     Assignment Preview
                  </h4>
                  <FilePreview
                     fileUrl={homework.fileUrl}
                  ...
                  <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                     ...
                     {submission.fileUrl && (
                        <div className="mt-4">
                           ...
                           <FilePreview
                              fileUrl={submission.fileUrl}
                              fileName={`${homework.title || "homework"}_submission`}
                           />
                        </div>
                     )}
                  </div>
