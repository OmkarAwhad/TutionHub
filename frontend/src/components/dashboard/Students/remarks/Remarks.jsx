	...
	useEffect(() => {
		const fetchRemarks = async () => {
			try {
				...
				let response;
				if (userId) {
					// Admin viewing specific student's remarks
					response = await viewRemarks(userId, token);
				} else {
					// Student viewing their own remarks - pass null as userId
					response = await viewRemarks(null, token);
				}
				if (response) {
					setRemarks(response);
				}
			...
			{remarks.length > 0 ? (
						...
						>
							...
							<div className="flex flex-col sm:flex-row sm:items-center gap-2 mb-3">
								<div className="flex items-center gap-2">
									...
									<span className="text-xs sm:text-sm text-medium-gray font-medium">
										Subject:
									</span>
								</div>
							...
							<div className="mb-3 sm:mb-4">
								<p className="text-xs sm:text-sm text-medium-gray font-medium mb-2">
									Remark:
								</p>
				...
				<div className="text-center py-8 sm:py-12">
					...
					<p className="text-slate-gray text-sm sm:text-base">
						Your remarks will appear here when added by tutors
					</p>
				</div>
			)}
		</div>
