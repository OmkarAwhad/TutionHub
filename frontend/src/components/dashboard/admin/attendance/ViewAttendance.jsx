...
import { FaArrowLeftLong, FaGraduationCap, FaFilter } from "react-icons/fa6";
...
function ViewAttendance() {
   ...
   const fetchLectures = async () => {
      try {
         const response = await dispatch(getLecturesWithAttendanceMarked(token));
         if (response) {
   ...
   return (
      <div className="p-4 sm:p-6">
         ...
         {/* Standards Filter and Desktop Filters */}
         <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 lg:gap-6 mb-6">
            ...
            <div className="hidden sm:flex items-center gap-3 lg:gap-4">
               {/* Subject Filter */}
               <div>
                  <label className="block text-xs text-slate-gray mb-1">Subject</label>
                  <select
                     value={selectedSubject}
                     onChange={(e) => setSelectedSubject(e.target.value)}
                     className="px-3 py-2 lg:px-4 lg:py-2 border-2 border-light-gray rounded-lg bg-white text-charcoal-gray font-medium focus:outline-none focus:border-charcoal-gray transition-all duration-200 min-w-[120px] lg:min-w-[140px] text-sm lg:text-base"
                  >
                     <option value="all">All Subjects</option>
                     {subjects.map((subject) => (
                        <option key={subject._id} value={subject._id}>
               ...
               <div>
                  ...
                  <select
                     value={selectedDesc}
                     onChange={(e) => setSelectedDesc(e.target.value)}
                     className="px-3 py-2 lg:px-4 lg:py-2 border-2 border-light-gray rounded-lg bg-white text-charcoal-gray font-medium focus:outline-none focus:border-charcoal-gray transition-all duration-200 min-w-[100px] lg:min-w-[120px] text-sm lg:text-base"
                  >
                     <option value="Lecture">Lecture</option>
                     <option value="Test">Test</option>
                  </select>
