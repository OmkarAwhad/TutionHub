      ...
      const fetchData = async () => {
         try {
            ...
            if (editLecture) {
               ...
               const [fromHours, fromMinutes, fromPeriod] = timeFrom.split(/[: ]/);
               const [toHours, toMinutes, toPeriod] = timeTo.split(/[: ]/);
               ...
               // Set form values
               setValue("date", new Date(editLecture.date).toISOString().split("T")[0]);
               setValue("tutor", editLecture.tutor._id);
               setValue("subject", editLecture.subject._id);
               setValue("standardId", editLecture.standard._id);
   ...
   const submitHandler = async (data) => {
      try {
         ...
         const result = await dispatch(
            updateLecture(editLecture._id, { ...data, time: timeString }, token)
         );
         if (result) {
