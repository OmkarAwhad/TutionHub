						...
						<div className="flex items-center space-x-8">
							<div className="flex-shrink-0">
								<div className="w-24 h-24 rounded-full bg-medium-gray flex items-center justify-center text-5xl text-white shadow-md">
									{user.role === ACCOUNT_TYPE.STUDENT && <PiStudentFill />}
									{user.role === ACCOUNT_TYPE.TUTOR && <GiTeacher />}
									{user.role === ACCOUNT_TYPE.ADMIN && <RiAdminLine />}
								</div>
							</div>
							<div className="flex-grow">
								<div className="flex justify-between items-start">
									<div className="space-y-2">
										...
										<p className="text-slate-gray text-sm">{user.email}</p>
									</div>
									<button
										onClick={() => setIsEditing(true)}
										className="flex items-center space-x-2 px-5 py-2.5 rounded-xl bg-medium-gray text-white font-medium hover:bg-charcoal-gray transition-all duration-300 hover:scale-105"
									>
										<span>Edit</span>
										<RiEdit2Fill />
									</button>
								...
								<div className="mt-6 grid grid-cols-2 gap-6">
									<div className="space-y-1">
										<p className="text-sm text-slate-gray font-medium">Phone Number</p>
									...
									<div className="space-y-1">
										<p className="text-sm text-slate-gray font-medium">Gender</p>
										<p className="text-charcoal-gray font-semibold">
											{user?.profile?.gender || (
