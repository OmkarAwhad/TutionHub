 ...
 * Shows upload button even when no file is selected
 *
 * @param {Object} props
 ...
 */
function EnhancedFileUploaderNoFileRequired({
	onFileSelect = () => {},
	onUpload = null,
	title = "Upload File",
	...
	const validateAndSetFile = (selectedFile) => {
		...
		if (acceptedFileTypes && acceptedFileTypes.length > 0) {
			...
			if (!acceptedFileTypes.includes(fileExtension)) {
				setErrorMessage(
					`Invalid file type. Accepted: ${formatAcceptedTypes()}`
				);
				return;
			}
		}
		...
		<div className={`w-full ${className}`}>
            ...
            ${
				disabled
					...
					: "hover:border-medium-gray cursor-pointer"
			}
         `}
			...
			{/* File upload button - always shown when onUpload is provided */}
			{onUpload && (
				<div className="mt-4">
					<button
						type="button"
...
export default EnhancedFileUploaderNoFileRequired;
